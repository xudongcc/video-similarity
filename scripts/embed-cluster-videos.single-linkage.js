...
function clusterEmbeddings(records, config) {
  ...
  const unionFind = new UnionFind(normalized.length);
  ...
  const clusters = Array.from(groups.values())
    .map((members, index) => ({
      clusterId: index + 1,
      size: members.length,
      representativeMd5: members[0].md5,
      members: members.sort((a, b) => a.md5.localeCompare(b.md5)),
    }))
